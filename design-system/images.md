...
3. Prefer shared `BackgroundImage` for purple section fills; avoid inventing new gradients as SoT backgrounds.  
   Exception: `market-hero` `atmosphere: "crypto"` uses `CryptoCoinField`; `atmosphere: "indices"` uses `IndicesField` (`public/images/indices/globe-v2.png`); `atmosphere: "energy"` uses `EnergyField` (`public/images/energy/*-v2.png`).  
